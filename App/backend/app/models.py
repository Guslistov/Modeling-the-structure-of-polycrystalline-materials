...
class DConv(nn.Module):
    ...
    def __init__(self, cnum_in, cnum_out, 
        ...
        # Уменьшаем силу спектральной нормализации
        self.conv_sn = nn.Conv2d(cnum_in, cnum_out, ksize, stride, padding)
        # Или можно оставить спектральную нормализацию, но уменьшить количество итераций
        # self.conv_sn = Conv2DSpectralNorm(cnum_in, cnum_out, ksize, stride, padding, n_iter=1)
        self.leaky = nn.LeakyReLU(negative_slope=0.1)  # Уменьшаем slope для более слабой активации
...
class Discriminator(nn.Module):
    """Более слабая версия дискриминатора"""

    def __init__(self, cnum_in, cnum):
        super().__init__()
        
        # Уменьшаем количество слоев и каналов
        self.conv1 = DConv(cnum_in, cnum)
        self.conv2 = DConv(cnum, cnum)  # Не увеличиваем каналы в 2 раза
        self.conv3 = DConv(cnum, 2*cnum)
        self.conv4 = DConv(2*cnum, 2*cnum)  # Не увеличиваем дальше каналы
