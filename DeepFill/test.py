...
model_path = "checkpoints/EBSD_IMAGES/states_NN.pth"
in_path_IMG = "examples/2_in.png"
in_path_EBSD = "examples/71-3.ctf"
out_file_name = "test32"
out_path_EBSD = "examples/" + out_file_name + ".ctf"
out_path_IPF = "examples/" + out_file_name + ".png"



def main():
    print("Старт...")
    #proccess_IPF(in_path_EBSD, 1)
    #process_euler_reconstraction(in_path_EBSD, out_path_EBSD, model_path, 2, resized=False)
    #proccess_IPF(out_path_EBSD, out_path_IPF, 2)
    #proccess_IPF(in_path_EBSD, out_path_IPF, 2)
    #proccess_eulers(in_path_EBSD, out_path_IPF)

    proccess_interlaced(in_path_EBSD, out_path_EBSD, 4)

    #process_euler_reconstraction(in_path_EBSD, out_path_EBSD, model_path, 2, resized=True)
    #proccess_eulers(in_path_EBSD, out_path_IPF)
    #proccess_IPF(in_path_EBSD, out_path_IPF, 0)

#---------------------------------Функция реконструкции углов эйлера------------------------------------
def process_euler_reconstraction(in_path, out_path, model_path, y_step, resized = False):
...
def generate(h,w,stride,feather,window_size,generator,device, image_inpainted,mask):
    for i in range(0, h, stride):
        for j in range(0, w, stride):
            ...
            if feather > 0:
                # Apply feathering only if not at image border
                if i > 0:
                    weight[..., :feather, :] *= torch.linspace(0, 1, feather, device=device).view(1, 1, feather, 1)
                if i_end < h:
                    weight[..., -feather:, :] *= torch.linspace(1, 0, feather, device=device).view(1, 1, feather, 1)
                if j > 0:
                    weight[..., :, :feather] *= torch.linspace(0, 1, feather, device=device).view(1, 1, 1, feather)
                if j_end < w:
                    weight[..., :, -feather:] *= torch.linspace(1, 0, feather, device=device).view(1, 1, 1, feather)
...
def load_EBSD_file(path_to_file):
    ...
    print(size_x,size_y,step_x,step_y)
...
def proccess_IPF(in_path, out_path, type):
    df, size_x, size_y, step_x, step_y = load_EBSD_file(in_path)

    #filename = in_path_EBSD.split('/')[-1].split('.')[0]
    #dir_path = '/'.join(in_path_EBSD.split('/')[:-1])
    #coord = "Z"
    #if type == 0:
    #    coord = "X"
    #elif type == 1:
    ...
    #elif type == 2:
    #    coord = "Z"
    #out_path = dir_path + "/" + filename + "_IPF_" + coord + ".png"

    create_IPF_image(df, type, out_path, size_x, size_y)
#------------------------------------------------------------------------------------------------
#---------------------------------Функция сжатия------------------------------------
def proccess_interlaced(in_path, out_path, y):
    df, size_x, size_y, step_x, step_y = load_EBSD_file(in_path)

    df = df[['Euler1', 'Euler2', 'Euler3']]
    data = df.values.astype(np.float32)
    image_data = data.reshape(size_y, size_x, 3)
    result = image_data[::y, :, :]  # Берём каждую 4-ю строку
    print(result.shape)
    size_x = result.shape[1]
    size_y = result.shape[0]
    flat_data = result.reshape(-1, 3)
    df = pd.DataFrame(flat_data, columns=["Euler1", "Euler2", "Euler3"])
...
def proccess_eulers(in_path, out_path):
    df, size_x, size_y, step_x, step_y = load_EBSD_file(in_path)
